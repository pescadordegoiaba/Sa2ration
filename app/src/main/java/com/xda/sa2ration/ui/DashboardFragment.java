...
import com.xda.sa2ration.ui.calibration.CalibrationActivity;
...
public final class DashboardFragment extends Fragment {
    ...
    private void renderSimple(DisplayConfiguration c,DisplayUiState state){
        ...
        LinearLayout buttons=new LinearLayout(requireContext());buttons.setGravity(Gravity.CENTER);boolean stacked=stackActions();buttons.setOrientation(stacked?LinearLayout.VERTICAL:LinearLayout.HORIZONTAL);
        Button apply=button("Aplicar");apply.setOnClickListener(v->viewModel.apply());buttons.addView(apply,actionParams(stacked));
        Button reset=button("Resetar");reset.setOnClickListener(v->{viewModel.resetAll();requireActivity().recreate();});buttons.addView(reset,actionParams(stacked));
        ...
        Button calibration=button(getString(R.string.open_calibration));calibration.setOnClickListener(v->startActivity(new Intent(requireContext(),CalibrationActivity.class)));content.addView(calibration);
    }
    ...
    private void info(String text){TextView view=new TextView(requireContext());view.setText(text);view.setPadding(dp(8),dp(8),dp(8),dp(12));view.setTextSize(13);view.setTextColor(MaterialColors.getColor(view,com.google.android.material.R.attr.colorOnSurfaceVariant,0xff555555));content.addView(view);}
    private void addTitle(String text){TextView title=new TextView(requireContext());title.setText(text);title.setTextSize(24);title.setTextColor(MaterialColors.getColor(title,com.google.android.material.R.attr.colorOnSurface,0xff111111));title.setPadding(dp(4),dp(4),dp(4),dp(16));content.addView(title);}
    private Button button(String label){MaterialButton button=new MaterialButton(requireContext());button.setText(label);button.setMinHeight(dp(48));button.setMaxLines(2);LinearLayout.LayoutParams params=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);params.setMargins(0,dp(3),0,dp(3));button.setLayoutParams(params);return button;}
    private boolean stackActions(){return getResources().getConfiguration().screenWidthDp<380||getResources().getConfiguration().fontScale>=1.2f;}
    private LinearLayout.LayoutParams actionParams(boolean stacked){LinearLayout.LayoutParams params=stacked?new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT):new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1);params.setMargins(dp(3),dp(3),dp(3),dp(3));return params;}
